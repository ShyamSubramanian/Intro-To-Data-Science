...
def eval_player(ID='hattesc01'):
    ...
    '''
    battingdf = pd.read_csv("Batting.csv",header=0)
    player = battingdf.loc[battingdf['playerID'] == ID]
    H = player.iloc[0]['H']
    AB = player.iloc[0]['AB']
    BB = player.iloc[0]['BB']
    HBP = player.iloc[0]['HBP']
    SF = player.iloc[0]['SF']
    _2B = player.iloc[0]['2B']
    _3B = player.iloc[0]['3B']
    HR = player.iloc[0]['HR']
...
def salary(ID='hattesc01'):
    '''
        ...
        Hint: you could use pandas package to load csv file and search player. Here is a tutorial: http://pandas.pydata.org/pandas-docs/stable/10min.html
    '''
    salarydf = pd.read_csv("Salaries.csv",header=0)
    player = salarydf.loc[salarydf['playerID'] == ID]
    S = player.iloc[0]['salary']
    return S 
