...
#--------------------------
    ...
    def mapper(self, in_key, in_value):
        ...
        if matrix_name == 'A':
            for n in range(1,nc+1):
                yield ('C',i,n),(matrix_name,i,j,v)
        else:
            for n in range(1,nr+1):
                yield ('C',n,j),(matrix_name,i,j,v)
    #----------------------
    def reducer(self, in_key, in_values):
        ...
        A = [item[3] for item in A]
        B = [item[3] for item in B]
        A = np.asmatrix(A)
        B = np.asmatrix(B)
        B = B.reshape(B.shape[1],B.shape[0])
        out_mat_val = np.matmul(A,B)
        yield in_key,float(out_mat_val[0,0])
