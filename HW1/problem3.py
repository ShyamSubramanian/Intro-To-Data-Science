...
def compute_P(A):
    ...
    numrows = A.shape[0]
    numcols = A.shape[1]
    # constructing diagonal matrix with sum of each column matrix A in diagonal
    diagonalmatrix = np.zeros((numrows,numcols))
    for j in range(numcols):
        columnsum = 0
        for i in range(numrows):
            columnsum += A[i,j]
        if columnsum != 0:
            diagonalmatrix[j,j] = 1/columnsum
    diagonalmatrix = np.asmatrix(diagonalmatrix)
    # normalizing the Adjacency matrix column-wise
    P = np.matmul(A,diagonalmatrix)
    return P
...
def random_walk(P, x_0, max_steps=10000):
    ...
    for i in range(max_steps):
        ...
        n_steps += 1
        if np.allclose(x,prev_x) is True:
            break
    return x,n_steps
#--------------------------
def pagerank_v1(A):
