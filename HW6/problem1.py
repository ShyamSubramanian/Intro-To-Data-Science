...
def compute_z(x,w,b):
    ...
    '''
    z = np.asscalar(np.matmul(w.T,x)) + b
    return z 
...
#-------------------------- -yloga-(1-y)log(1-a) ==> -0*log(1)-1*log(0) ==> -(-inf) = inf  
def compute_L(a,y):
    '''
...
def backward(x,y,a):
    ...
    '''
    dL_da, da_dz, dz_dw, dz_db = compute_dL_da(a,y), compute_da_dz(a), compute_dz_dw(x), compute_dz_db()
