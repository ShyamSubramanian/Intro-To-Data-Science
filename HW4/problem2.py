...
def reshape_to_image(x):
    ...
    (5 points)
    '''
    numrows = numcols = int(np.sqrt(x.shape[1]))
    image = np.reshape(x,(numrows,numcols))
    return image
...
def compute_mu_image(X):
    ...
    '''
    mu_image = np.asmatrix(np.mean(X,axis=0))
...
def compute_eigen_faces(X, k=20):
    ...
    Xp, P_images = p1.PCA(X,k)
    P_images = [reshape_to_image(P_images[:,i].T) for i in range(k)]
    return P_images, Xp
...
def load_dataset():
    ...
    images = faces.images
    X = np.asmatrix(faces.data)
    y = np.asmatrix(faces.target).T
